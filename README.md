...
Для запуска всех сервисов проекта используется Docker Compose. В одной команде вы сможете запустить backend, frontend и базу данных.

В корне проекта расположен файл docker-compose.yml, который описывает конфигурацию всех сервисов.
...
Особенности работы:
- Все сервисы находятся в одной виртуальной сети Docker, поэтому backend может подключаться к базе по имени сервиса.
- Порты для доступа к frontend и backend проброшены на локальную машину.
- Данные базы хранятся в Docker volume, чтобы сохранить состояние между перезапусками.
